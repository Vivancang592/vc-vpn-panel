...
if (!is_array($selectedGroupIds)) {
    ...
    if (!is_array($selectedGroupIds)) {
        $selectedGroupIds = !empty($plan['group_id']) ? [(int)$plan['group_id']] : [];
    }
}
                ...
                <?php if (!empty($groups)): ?>
                    <?php foreach ($groups as $group): ?>
                        <label style="display: inline-flex; align-items: center; gap: 0.4rem; cursor: pointer; font-size: 0.9rem;">
                            <input type="checkbox" name="group_ids[]" value="<?= $group['id'] ?>" <?= in_array($group['id'], $selectedGroupIds) ? 'checked' : '' ?> style="cursor: pointer; width: 16px; height: 16px;">
