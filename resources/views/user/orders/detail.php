...
<section class="user-orders-page">
	...
	<article class="glass-card user-invoice-card">
		<div class="user-invoice-document-head">
			<div><span class="user-invoice-document-label">BÊN CUNG CẤP</span><strong><?= htmlspecialchars($settings['site_title'] ?? 'VC VPN 2027') ?></strong></div>
			<div><span class="user-invoice-document-label">SỐ HÓA ĐƠN</span><strong>INV-<?= str_pad((string) $orderId, 6, '0', STR_PAD_LEFT) ?></strong></div>
		</div>
...
<style>
.user-invoice-header { align-items: center; flex-direction: column; gap: 10px; padding: 10px 0 26px; text-align: center; }
.user-invoice-header h2 { margin: 0; color: #172033; font-family: Georgia, serif; font-size: 1.7rem; }.user-invoice-header p:last-child { margin: 7px 0 0; color: var(--ios-gray); }
.user-invoice-kicker { margin: 0 0 8px; color: #0b57d0; font-size: .76rem; font-weight: 800; letter-spacing: .12em; }
...
.user-invoice-card { max-width: 860px; margin: 0 auto; padding: 0; overflow: hidden; border: 1px solid rgba(46, 68, 100, .16); box-shadow: 0 16px 42px rgba(36, 52, 80, .12); }.user-invoice-document-head { display: flex; justify-content: space-between; gap: 24px; padding: 24px 30px; background: #edf4ff; border-bottom: 1px solid #cbdcf5; }.user-invoice-document-head > div:last-child { text-align: right; }.user-invoice-document-head strong { display: block; color: #172033; font-size: 1.05rem; }.user-invoice-document-label { display: block; margin-bottom: 5px; color: #52647e; font-size: .7rem; font-weight: 800; letter-spacing: .08em; }.user-invoice-total { display: flex; justify-content: space-between; align-items: center; margin: 26px 30px 0; padding: 18px 20px; background: #102a43; color: #fff; border-radius: 6px; }.user-invoice-total span { font-size: .9rem; font-weight: 600; opacity: .78; }.user-invoice-total strong, .user-invoice-money { color: #0b57d0; font-size: 1.3rem; }.user-invoice-total strong { color: #fff; font-size: 1.45rem; }.user-invoice-rows { padding: 0 30px 22px; }.user-invoice-section-heading { margin: 30px 0 6px; padding-bottom: 9px; color: #172033; font-size: .82rem; font-weight: 800; letter-spacing: .05em; text-transform: uppercase; border-bottom: 2px solid #d8e5f7; }.user-invoice-row { display: grid; grid-template-columns: minmax(190px, .85fr) minmax(0, 1.15fr); gap: 28px; padding: 12px 14px; border-bottom: 1px solid rgba(90, 105, 125, .12); align-items: center; }.user-invoice-row:nth-of-type(even) { background: rgba(235, 241, 249, .52); }.user-invoice-row span { color: #66768b; font-size: .9rem; }.user-invoice-row strong { color: #172033; text-align: right; overflow-wrap: anywhere; }.user-invoice-money { font-weight: 800; }.user-invoice-status-value { justify-self: end; padding: 4px 9px; border-radius: 5px; font-size: .82rem; }.user-invoice-status-value.is-pending { background: #fff3d4; color: #9a6200; }.user-invoice-status-value.is-completed { background: #dff7e8; color: #197447; }.user-invoice-status-value.is-failed, .user-invoice-status-value.is-cancelled { background: #ffe3e3; color: #b32929; }.user-invoice-actions { margin: 4px 30px 30px; text-align: right; }
@media (max-width: 700px) { .user-invoice-header h2 { font-size: 1.4rem; }.user-invoice-card { padding: 0; }.user-invoice-document-head { padding: 20px; gap: 12px; }.user-invoice-document-head strong { font-size: .92rem; }.user-invoice-total { margin: 20px 18px 0; padding: 16px; }.user-invoice-rows { padding: 0 18px 18px; }.user-invoice-row { grid-template-columns: minmax(0, 1fr) minmax(0, 1.2fr); gap: 14px; padding: 12px 8px; }.user-invoice-row strong { font-size: .88rem; }.user-invoice-total strong { font-size: 1.12rem; }.user-invoice-actions { margin: 4px 18px 22px; } }
</style>
